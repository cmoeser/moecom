...
var _GlobalNavigationEs6 = require('./GlobalNavigation.es6');

var _GlobalNavigationEs62 = _interopRequireDefault(_GlobalNavigationEs6);

var UniversalSubnavigation = (function () {
    function UniversalSubnavigation($, Utils) {
        _classCallCheck(this, UniversalSubnavigation);

        var _this = this;

        var utils = new Utils();
        var globalNav = new _GlobalNavigationEs62['default']($, Utils);
        ...
        var scrollContainerScrollX = 0; // cache the scroll Container scroll

        // Test if webkit-overflow-scrolling is supported
        var webkitOverflowScrollingSupport = utils.webkitOverflowScrollingSupport();
        ...
        this.positionFixedSubNav = function ($currentNavItem) {
            if (utils.getViewportSize() === 'medium' && $currentNavItem.length > 0) {
                ...
                var margin = parseInt($secondaryNav.css('marginLeft'), 10);
                var left = $currentNavItem[0].getBoundingClientRect().left - margin - 30;

                // set the leftMax to prevent positioning of subNav off the screen
                var leftMax = $window.width() - margin * 2 - $currentNavItem.children('.' + selectors.subNav).width() - 20;
        ...
        var setMinWidthSubNav = function setMinWidthSubNav($currentNavItem) {
            var minWidth = $currentNavItem.children('a').width();
        ...
        this.centerScrollNav = function (duration) {
            if (utils.getViewportSize() !== 'large' && $scrollContent.width() > $scrollContainer.width()) {
                var $activeNavItem = $navItems.filter('.' + selectors.active),
        ...
        this.bindMainEventListeners = function () {
            ...
            $navItems.on('touchstart mouseenter', function (e) {
                ...
                if (utils.getViewportSize() !== 'small') {
                    (function () {
                        ...
                        var $currentNavItem = $(_this2);

                        if ($currentNavItem.hasClass(selectors.dropdown)) {

                            if (e.type == 'touchstart' && webkitOverflowScrollingSupport) {
                                // iOS fix for no mouseleave event
                                ...
                                $(document).on('touchstart.navItem', function (e) {
                                    // allow user to close subnav by tapping outside
                        ...
                        $(_this2).addClass(dynamicClasses.hoverOpen);
                    })();
                }
            ...
            $('.dropdown').on('click', function () {
                //on mobile click, open a full-screen flyout
                if (utils.getViewportSize() === 'small') {
            ...
            $window.on('resize', function () {
                utils.debounce(function () {
                    ...
                    var $openNavItem = $navItems.filter('.' + dynamicClasses.hoverOpen);

                    if ($openNavItem.length > 0) {
                        window.setTimeout(function () {
                            // give time for orientation to update
        ...
        this.bindAccessibilityListeners = function () {
            ...
            $navItemAs.on('focus', function () {
                var $currentNavItem = $(this).parent('.' + selectors.navItem);
                if (utils.getViewportSize() !== 'small') {

                    var delay = 0; // no delay for desktop

                    if (utils.getViewportSize() !== 'large') {
                        delay = 200; // set delay for tablet
                    }
                    setTimeout(function () {
                        // in case the focused item is off screen, allow the browser to bring it on screen
                        $currentNavItem.addClass(dynamicClasses.hoverOpen).siblings().removeClass(dynamicClasses.hoverOpen);
                        _this.positionFixedSubNav($currentNavItem);
                    }, delay);
                } else if ($navItems.filter('.' + dynamicClasses.subNavOpen).length > 0) {
                    // if user does not tab into flyout

                    window.setTimeout(function () {
                        // timeout allows activeElement to update
            ...
            $subNavs.on('focusout', function () {
                var $currentParentNavItem = $(this).parents('.' + selectors.navItem);
    ...
    _createClass(UniversalSubnavigation, [{
        key: 'name',
        value: function name() {
        ...
        value: function setWidthScrollContainer(bool) {
...
exports['default'] = UniversalSubnavigation;
